...
读 `INDEX.json`（缺失或明显过期时先跑 `build_index.py` 重建）。用语义匹配而非纯关键词：综合 `chart_types` + `data_shape` + `use_when`/`not_when` + `aliases` 判断哪条记录符合用户当前的数据和意图。
...
3. **按目的裁剪**：用户目的与条目 use_when 有偏差时，只借用的确服务当前目的的部分（借图层结构、借配色逻辑、借联动设计……），其余按需取舍；条目细节与用户明确要求冲突时一律以用户为准
4. **交付说明借用关系**：说明"参考了 `NNN-slug` 的哪些方面、按你的数据改了哪些"，让用户知道哪些结果是临摹、哪些是适配决策
...
- 用户对命中条目的结果不满意 → 修改的是**图库条目本身**（更新配方/模板），而不是只改一次性代码；改完重跑模板自检并更新索引
